...
Sub-minimum residuals left by rounded sells are dropped from the ledger so they
cannot occupy a rotation slot (2026-09-18). Optional, default-off sizing flags
`--slot-top-up`, `--cash-reserve`, `--halt-peak-dd` and `--halt-daily-loss`
exist; the top-up profile was replayed and not promoted, see
