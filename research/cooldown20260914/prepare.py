"""Freeze an offline Engine replay of 72h versus 120h post-sale cooldown."""
import argparse
...
def main():
    ...
    budget, max_order = Decimal(args.budget), Decimal(args.max_order)
    if not budget.is_finite() or not max_order.is_finite() or not 0 < max_order <= budget:
        raise ValueError('finite positive budget and order cap are required')
    ...
    for name in ('engine.go', 'market.go', 'signals.go', 'client.go'):
        ...
        if name == 'engine.go':
            assert 's.Cooldown[o.Symbol] = replayNow().Add(e.Config.postSaleCooldown())' in text
        target = out/(name+'.txt')
    ...
    text = template.read_text().replace('replayBonus', 'replayExtendedCooldown').replace('Bonus', 'ExtendedCooldown')
    old = 'cfg:=DefaultConfig();cfg.Budget'
    assert text.count(old) == 1
    text = text.replace(old, 'cfg:=DefaultConfig();cfg.CooldownHours=72+48*int(bonus);cfg.Budget')
    text = text.replace('cfg.Budget=decimal.NewFromInt(495)',
                        f'cfg.Budget=decimal.RequireFromString("{budget}")')
    text = text.replace('cfg.MaxOrder=decimal.NewFromInt(49)',
                        f'cfg.MaxOrder=decimal.RequireFromString("{max_order}")')
    old = 'range []int{0,28}'
    assert text.count(old) == 1
    ...
    (out/'config.json').write_text(json.dumps(dict(budget=str(budget),
        max_order=str(max_order), geometries=args.geometries), indent=2)+'\n')
