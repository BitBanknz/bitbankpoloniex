...
def family():
    ...
    for lookback in [20,60,120]:
        for slots in [1,3]:
            out.append(dict(name=f'trend{lookback}-top{slots}',kind='trend',lookback=lookback,slots=slots,short=False))
    for leaves in [7,15]:
        out.append(dict(name=f'lightgbm{leaves}',kind='lgb',leaves=leaves,slots=3,short=False))
    return out
...
def simulate(weights,data,start,end,fee=.003,borrow=.0003,eth=None):
    ...
    n=op.shape[1];positions=np.zeros(n);cash=1.;equity=1.;rets=[];turnover=0.;liquidations=0;borrow_paid=0.;trades=0
...
def main():
    ...
    declaration={'schema':'nested-daily-v1','candidates':candidates,'outer_boundaries':[str(grid[i]) for i in boundaries],'final_holdout':[str(grid[holdout_start]),str(grid[final])],'sources':sources,'cost_per_side':.003,'borrow_daily_scenario':.0003,'warning':'current-listed universe has survivorship bias; historical collateral/borrow availability unknown; margin results are scenarios only'}
    ...
    for fold,(start,end) in enumerate(zip(boundaries[:-1],boundaries[1:])):
        ...
        for candidate in candidates:
            ...
            objective=m['return']-1.5*m['max_drawdown'];scores.append((objective,candidate,m))
    ...
    for c in candidates:
        ...
        try:_,m=simulate(w,data,inner_start,holdout_start);objective=m['return']-1.5*m['max_drawdown']
    ...
    for name,collateral,margin in [('cash',False,False),('existing_eth_collateral',True,False)]:
        ...
        r,m=simulate(w,data,holdout_start,final);_,stress=simulate(w,data,holdout_start,final,fee=.006,borrow=.001)
        scenarios[name]={'normal':m,'stress':stress};curves[name]=np.cumprod(1+r).tolist()
    hold=np.zeros_like(y);hold[:,eth]=1.;r,benchmark=simulate(hold,data,holdout_start,final,fee=.003)
    ...
    result={'outer_cv':cv,'positive_outer_folds':positive,'positive_stress_folds':stress_positive,'folds':cv_rows,'final_candidate':chosen,'final_holdout':scenarios,'hold_eth_benchmark':benchmark,'research_gate_passed':passed,'runtime_promoted':False,'limits':declaration['warning']+'; daily candle fills lack historical spreads/order books; prior BitBank studies consumed some overlapping dates, so this is procedurally sealed, not globally untouched evidence'}
